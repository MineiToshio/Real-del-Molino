		...
		$.ajax({
			...
			success: function (html) {				
				...
				if (html==1) {					
					...
					//alert('El mensaje ha sido enviado satisfctoriamente.');	
					jAlert('El mensaje ha sido enviado satisfctoriamente.', 'Envio de mensaje');
					$('input[name=name]').val('');
					$('input[name=email]').val('');
					$('input[name=subject]').val('');
					$('textarea[name=message]').val('');
				//if process.php returned 0/false (send mail failed)
				} else alert('Ha ocurrido un error inesperado. Por favor intentar en unos momentos.');				
			}		
		});
